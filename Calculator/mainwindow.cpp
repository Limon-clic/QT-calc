...
void MainWindow::read()                                         // читаем из Edit и делаем предидущую мат. операцию
{
    ...
    try
    {
        calc.set_x(calc.calc(a));                               // выполняем пред. операцию (если была) и записываем результат в x
    }
...
void MainWindow::on_Button_equals_clicked()                     // равно - показать ответ
{
    ...
    calc.set_op(NONE);                                          // сбрасываем мат. операцию для дальнейшей работы

    ui->Edit_screen->setText(QString::number(calc.get_x()));    // выводим результат вычислений
}
...
void MainWindow::on_Button_plus_clicked()                       // сложение
{
    ...
    calc.set_op(PLUS);                                          // запоминаем мат. операцию
}
...
void MainWindow::on_Button_minus_clicked()                      // вычитание
{
    ...
    calc.set_op(MINUS);                                         // запоминаем мат. операцию
}
...
void MainWindow::on_Button_mult_clicked()                       // умножение
{
    ...
    calc.set_op(MULT);                                          // запоминаем мат. операцию
}
...
void MainWindow::on_Button_division_clicked()                   // деление
{
    ...
    calc.set_op(DIVISION);                                      // запоминаем мат. операцию
}
...
void MainWindow::on_Button_power_clicked()                      // возв. в степень
{
    ...
    calc.set_op(POW);                                           // запоминаем мат. операцию
}
...
void MainWindow::on_Button_sin_clicked()                        // синус
{
    ...
    calc.set_op(SIN);                                           // запоминаем мат. операцию
}
...
void MainWindow::on_Button_cos_clicked()                        // косинус
{
    ...
    calc.set_op(COS);                                           // запоминаем мат. операцию
}
...
void MainWindow::on_Button_tan_clicked()                        // тангенс
{
    ...
    calc.set_op(TAN);                                           // запоминаем мат. операцию
}
...
void MainWindow::on_Button_exp_clicked()                        // експонента
{
    ...
    calc.set_op(EXP);                                           // запоминаем мат. операцию
}
...
void MainWindow::on_Button_ln_clicked()                         // натуральный логарифм
{
    ...
    calc.set_op(LN);                                            // запоминаем мат. операцию
}
...
{
    ui->Edit_screen->setText("");                               // очищаем поле для дальнейших расчётов
    calc.set_oll(0,PLUS);                                       // устанавливаем значения по умолчанию
}
...
/*___________Конец цыферок___________*/
